  ...
  for(k in 1:length(uni.clus)){
      if(f != k){
        ...
        for(i in 1:dim(clus1)[1]){
          ...
          gvert<-  which.min(dtemp1[1:dim(clus2)[1]])
          clus1.mat[i,1]<-V(gr2)$UNINDEX[i]
          clus1.mat[i,2]<-V(gr3)$UNINDEX[gvert]
          }
        Sys.time()-t1  
        assign(paste("NTcluster",f,k,sep="."),clus1.mat)
        print(k)
        } else
        print(c(f, k))
...
source( "f:/foragefish/HydroNetworks/Trunk/ClustAggregator.r")

source( "f:/foragefish/HydroNetworks/Trunk/WriteLinesARCclust.r")
library(igraph)
library(foreign)
source( "C:/data/Pierce/foragefish/HydroNetworks/Trunk/NetFunctions.r")

wria <- c("PS")
LINEID <- c("OBJECTID") #wchydro is LLID
LENGTH <- c("Shape_Length") #wchydro is LLID
library(RODBC)
cnn1 <- odbcConnectAccess("C:/data/Pierce/ForageFish/PugetSoundNet.mdb")
...
dim(linesnet.filt)
table.norow(linesnet.filt[,3:4],paste("C:/data/Pierce/foragefish/PSnet.GR",wria,".txt",sep=""),col.names=F)
##################################################################################


##Import Graph and add identifier attributes
graphset<- read.graph(paste("C:/data/Pierce/foragefish/PSnet.GR",wria,".txt",sep=""))

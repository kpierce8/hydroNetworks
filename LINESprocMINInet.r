...
LENGTH <- c("Shape_Leng") #wchydro is LLID
#library(RODBC)
#cnn1 <- odbcConnectAccess("g:/ForageFish/PugetSoundNet.mdb")
#lines.dbf <- sqlFetch(cnn1,"PSnet")
#nodes.dbf<- sqlFetch(cnn1,"PugetSoundNet_Net_Junctions")
nodes.dbf <-read.dbf("e:/ForageFish/NetworkTempFiles/mininodes.dbf")
lines.dbf <-read.dbf("e:/ForageFish/NetworkTempFiles/mininet.dbf")
...
dim(linesnet.filt)
table.norow(linesnet.filt[,3:4],paste("e:/foragefish/PSnet.GR",wria,".txt",sep=""),col.names=F)
##################################################################################


##Import Graph and add identifier attributes
graphset<- read.graph(paste("e:/foragefish/PSnet.GR",wria,".txt",sep=""))
graphset <- set.edge.attribute(graphset,"length",E(graphset),linesnet.attr[,lencol])
graphset <- set.edge.attribute(graphset,"weight",E(graphset),linesnet.attr[,lencol])
graphset <- set.edge.attribute(graphset,"OBJECTID",E(graphset),linesnet.attr[,idcol])
...
plot(gr2,layout=layout.fruchterman.reingold,vertex.size=7)
gBlocks <- cohesive.blocks(gr2)

plot(gBlocks,layout=layout.fruchterman.reingold,vertex.size=7)

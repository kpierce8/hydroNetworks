library(igraph)
library(foreign)
source( "f:/foragefish/HydroNetworks/Trunk/NetFunctions.r")

wria <- c("PS")
LINEID <- c("OBJECTID") #wchydro is LLID
LENGTH <- c("Shape_Length") #wchydro is LLID
library(RODBC)
cnn1 <- odbcConnectAccess("f:/ForageFish/PugetSoundNet.mdb")
...
dim(linesnet.filt)
table.norow(linesnet.filt[,3:4],paste("f:/foragefish/PSnet.GR",wria,".txt",sep=""),col.names=F)
##################################################################################


##Import Graph and add identifier attributes
graphset<- read.graph(paste("f:/foragefish/PSnet.GR",wria,".txt",sep=""))
